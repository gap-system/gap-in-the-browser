...
##  <P/>
##  In the following situations, a nonempty default value is computed.
##  If the current &GAP; session is running inside a &Julia; session
##  then a &Julia; scratchspace is taken.
##  Otherwise, if the subdirectories <F>dataext</F>, <F>datagens</F>,
##  <F>dataword</F> of the &AtlasRep; directory are writable for the user
##  then the installation path of the &AtlasRep; package (including a
##  trailing slash symbol) is taken.
##  In all other cases, the default is an empty string.
##  </Subsection>
##  <#/GAPDoc>
##
DeclareUserPreference( rec(
...
If the current GAP session is running inside a Julia session \
then a Julia scratchspace is taken. \
Otherwise, if the subdirectories 'dataext', 'datagens', \
'dataword' of the AtlasRep directory are writable for the user \
then the installation path of the AtlasRep package (including a \
trailing slash symbol) is taken. \
In all other cases, the default is an empty string."
    ],
  default:= function()
      local val, dir, subdir, file;
      ...
      if IsPackageLoaded( "JuliaInterface" )
         and IsBound( GetJuliaScratchspace ) then
        # Create/fetch a Julia scratchspace.
        val:= ValueGlobal( "GetJuliaScratchspace" )( "atlasrep_cache" );
      ...
      if ForAll( [ "dataext", "datagens", "dataword" ],
        ...
        # The package directory can be taken as the default.
        return Filename( dir, "" );
      fi;

      # We do not know a reasonable default.
      return "";
    end,
  package:= "AtlasRep",
  check:= function( val )
...
##  JSON format file that contains the table of contents of this part,
