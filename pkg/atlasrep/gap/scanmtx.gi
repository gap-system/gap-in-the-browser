...
#F  FFLogList( <F> )
##
BindGlobal( "FFLogList", function( F )
...
#F  IntegerStrings( <q> )
##
BindGlobal( "IntegerStrings", function( q )
    local i;
...
##
BindGlobal( "CMeatAxeFileHeaderInfo", function( string )
    ...
    if fail in lline then
      ...
      # The header is of old type, consisting of three or four integers.
      if Length( lline ) = 3 and lline[1] = 13 then
        # Support an undocumented old type for monomial permutations.
        lline:= [ 13, lline[2], lline[3], 1 ];

      elif Length( lline ) = 3 and lline[1] = 12 then
...
##
BindGlobal( "ScanMeatAxeFile", function( arg )
    ...
    if mode in [ 2, 13, 12, 8 ] then

      # a permutation, to be converted to a matrix,
      # or a monomial permutation,
      ...
      if mode = 12 then
        ...
        # mode = 13:
        # a monomial permutation
        degree:= line[3] * ( line[2] - 1 );
        result:= [];
        if Length( string ) <> degree * line[4] then
          Info( InfoCMeatAxe, 1, "corrupted file, wrong number of entries" );
          return fail;
        ...
        if line[4] = 1 then
          # This is the usual case (avoid duplicating the list).
          perm:= [];
          for i in [ 1 .. line[3] ] do
            img:= string[ 2*i-1 ];
            scl:= string[ 2*i ];
            if scl = 1 then
              perm[i]:= img;
              perm[ i + line[3] ]:= img + line[3];
            else
              perm[i]:= img + line[3];
              perm[ i + line[3] ]:= img;
            fi;
          od;
          result[1]:= PermListWithTest( perm );
        else
          offset:= 0;
          for j in [ 1 .. line[4] ] do
            result[j]:=
...
##
BindGlobal( "CMtxBinaryFFMatOrPerm", function( arg )
...
##
BindGlobal( "FFMatOrPermCMtxBinary", function( fname )
...
##
BindGlobal( "ScanStraightLineDecision",
...
##
BindGlobal( "ScanStraightLineProgram", function( arg )
...
##
BindGlobal( "AtlasStringOfProgram", function( arg )
